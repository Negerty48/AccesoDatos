 ...
 */
package com.ejercicios.bbdd.ejercicio05;

import com.ejercicios.bbdd.ejercicio04.Empleado;
import java.sql.*;
...
public class Proyecto {
    ...
    public ArrayList<Empleado> getListAsigEmpleados(int num_proy) throws SQLException {
        ArrayList<Empleado> empleados = new ArrayList<>();
        String sql = "select DNI_NIF, NOMBRE from EMPLEADOS where DNI_NIF in"
                + "(select DNI_NIF_EMP from ASIG_PROYECTOS where NUM_PROY = ? "
                + "and F_INICIO < now() and (F_FIN is null OR F_FIN > now()));";
        ResultSet rs = null;
        try (Connection con = DriverManager.getConnection(URL, USER, PWD);
                PreparedStatement ps = con.prepareStatement(sql);) {
            ps.setInt(1, num_proy);
    ...
    public static void main(String[] args) {
        try {
            ...
            ArrayList<Empleado> e = p.getListAsigEmpleados(10);
            for (Empleado empleado : e) {
                System.out.println(empleado.getDni() + " || " + empleado.getName());
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
